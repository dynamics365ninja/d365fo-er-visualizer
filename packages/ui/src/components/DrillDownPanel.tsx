...
import { useAppStore, resolveDeepExpression, selectMappingDefinition, getScopedMappingDefinitions, toModelRootedPath } from '../state/store';
...
function buildTreeNode(
  ...
  // The format's model datasource can be named anything (`Invoice` in the
  // PEPPOL formats); model lookups always go through the `model` root.
  const rooted = toModelRootedPath(expression, ctx.configurations, configIndex);
  const isModel = rooted !== null;
  const cleanPath = rooted ? extractModelPath(rooted.modelExpression) : '';
  ...
  // under the name the format gave it.
  const formatDeep = rooted
    ? resolveDeepExpression(rooted.root + cleanPath.slice('model'.length), ctx.configurations, configIndex, ctx.scopeConfigIndex)
    : null;
